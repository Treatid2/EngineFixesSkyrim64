...
#define EF_SETTINGS_FIXES(X)                                                                                                                                                                                                                                      \
    ...
    X(Bool, bBatchRendererRenderPassArrayUAF, true, "guards BSBatchRenderer render-pass array accesses through a cleared or null-derived pass-list pointer")                                                                                                      \
    ...
    X(Bool, bGCArrayCleanupBug, true, "fixes the Papyrus VM's incremental garbage collector sometimes exiting its array/object cleanup loop after a single entry, forgoing the rest of its time budget and causing progressive script lag/hitches; skipped on VR") \
